...
def configure_proxmox_iptables():
    ...
    iptables_commands = [
        ...
        f"iptables -t nat -A PREROUTING -p tcp -d {proxmox_external_ip} --dport {PROMETHEUS_PORT} -j DNAT --to-destination {monitoring_vm_ip}:{PROMETHEUS_PORT}",
        ...
        # Allow forwarded Prometheus traffic
        f"iptables -A FORWARD -p tcp -d {monitoring_vm_ip} --dport {PROMETHEUS_PORT} -m state --state NEW,ESTABLISHED,RELATED -j ACCEPT",
        f"iptables -A FORWARD -p tcp -s {monitoring_vm_ip} --sport {PROMETHEUS_PORT} -m state --state ESTABLISHED,RELATED -j ACCEPT",
