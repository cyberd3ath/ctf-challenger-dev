...
def setup_wazuh_manager():
    ...
    commands = [
        ...
        f"sudo /var/lib/wazuh/manager/set_up_manager.sh --api-user {WAZUH_API_USER} --api-pass {WAZUH_API_PASS} --dashboard-user {WAZUH_DASHBOARD_USER} --dashboard-pass {WAZUH_DASHBOARD_PASS} --indexer-user {WAZUH_INDEXER_USER} --indexer-pass {WAZUH_INDEXER_PASS}",
    ...
    for cmd in commands:
        execute_remote_command_with_key(MONITORING_IP, cmd, SSH_USER, ssh_key_path=PROXMOX_SSH_KEYFILE, shell=True, timeout=1800)
