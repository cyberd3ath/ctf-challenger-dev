...
def connect_to_monitoring():
    """Connect monitoring VM to bridge network"""
    ...
    # Wait for interface to appear and configure it
    ...
    # Configure IP address and routing on remote host
    commands = [
        f"sudo ip -6 addr add {WAZUH_MANAGER_IPV6} dev {local_device_name}",
        f"sudo ip -6 route add default via {WAZUH_NETWORK_DEVICE_IPV6}"
    ]

    for cmd in commands:
        execute_remote_command_with_key(MONITORING_IP, cmd, SSH_USER, ssh_key_path=PROXMOX_SSH_KEYFILE)

    log_success("Monitoring VM connected to bridge network")
